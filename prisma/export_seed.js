...
const D = (d) => `new Date(${q(new Date(d).toISOString())})`;
async function main() {
    ...
    if (entities.length > 0) {
        ...
        for (const e of entities) {
            ...
            if (e.userId)
                L.push(`${I(3)}userId: ${uV[e.userId] ?? q(e.userId)}.id,`);
            L.push(`${I(2)}}});`);
        }
        L.push(``);
    }
    ...
    if (collections.length > 0) {
        ...
        for (const col of collections) {
            ...
            if (bv)
                L.push(`${I(3)}branchId: ${bv}.id,`);
            if (dav)
                L.push(`${I(3)}debitAccountId: ${dav}.id,`);
            if (cav)
                L.push(`${I(3)}creditAccountId: ${cav}.id,`);
            if (jev)
                L.push(`${I(3)}journalEntryId: ${jev}.id,`);
